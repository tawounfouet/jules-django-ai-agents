...
def decision_node(state: AgentState, config: RunnableConfig):
    ...
    try:
        ...
        return {
            "next_step": "support_agent",
            "messages": [AIMessage(content=f"Error loading decision agent: {e}")],
        }
    ...
    if "ROUTING_TO_DATA" in response.content:
        return {
            ...
            "messages": [AIMessage(content="Routing to Data Agent...")],
        }
    else:
        return {
            "next_step": "support_agent",
            "messages": [AIMessage(content="Routing to Support Agent...")],
        }
...
def data_agent_node(state: AgentState, config: RunnableConfig):
    ...
    try:
        ...
        return {
            "messages": [AIMessage(content=f"Error loading data agent: {e}")],
            "next_step": "end",
        }
...
def support_agent_node(state: AgentState, config: RunnableConfig):
    ...
    try:
        ...
        return {
            "messages": [
                AIMessage(
                    content="I am the Support Agent. I can help with general questions."
                )
            ],
            "next_step": "end",
        }
...
def document_agent_node(state: AgentState, config: RunnableConfig):
    """
    Handles document management tasks. Uses 'document_agent' configuration and dynamic tools.
    Supports: search, list, get, create, update, delete documents.
    """
    messages = state["messages"]
    try:
        model = get_llm("document_agent")
        ...
        return {
            "messages": [AIMessage(content=f"Error loading document agent: {e}")],
            "next_step": "end",
        }
    ...
    if response.tool_calls:
        ...
        for tool_call in response.tool_calls:
            ...
            if tool_name in tool_map:
                tool_func = tool_map[tool_name]
                # Invoke the tool with config for user context
                try:
                    result = tool_func.invoke(tool_call["args"], config=config)
                except Exception as tool_err:
                    result = f"Error executing {tool_name}: {tool_err}"
