...
def seed_agents_data(db):
    ...
    decision_agent = Agent.objects.create(
        key="decision_agent", name="Decision", role="Routing", 
        llm_provider="openai", llm_model="gpt-4o", temperature=0.0
    )
    data_agent = Agent.objects.create(
        key="data_agent", name="Data", role="Retrieval",
        llm_provider="openai", llm_model="gpt-4o", temperature=0.0
    )
    support_agent = Agent.objects.create(
        key="support_agent", name="Support", role="Support",
        llm_provider="openai", llm_model="gpt-4o", temperature=0.7
    )
    
    # Create Tools
    check_order = Tool.objects.create(
        key="check_order_status", name="Check Order", 
    ...
    def side_effect(key):
        ...
        if key == "decision_agent":
            ...
            m.invoke.return_value = AIMessage(
                content="", 
                tool_calls=[{"name": "check_order_status", "args": {"order_id": 999}, "id": "call_1"}]
            )
        return m
